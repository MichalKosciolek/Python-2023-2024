...
import zadanie1_bubble_sort as bs
import zadanie2_shell_sort as ss
import zadanie3_merge_sort as ms
import zadanie4_quick_sort as qs
import numpy as np
...
def main():
    ...
    tablica = MonitorowanaTablica(0, 1000, N, "T")  # Explore other options: "S", "A", "T"

    # Perform the sorting
    t0 = time.perf_counter()
    qs.quick_sort(tablica)
    delta_t = time.perf_counter() - t0

    print(f"Sorting: Quick")
    print(f"Array sorted in {delta_t*1000:.1f} ms. Number of operations: {len(tablica.pelne_kopie):.0f}.")

    # Plot the sorting animation
    plot_sorting_animation(tablica, "Quick", FPS)
################################################################
